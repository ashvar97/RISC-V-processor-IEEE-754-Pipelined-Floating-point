...
int sc_main(int argc, char* argv[]) {
    ...
    subtractor.A(A);
    subtractor.B(B);
    subtractor.result(result);
    subtractor.valid_out(valid_out);
    subtractor.overflow(overflow);
    subtractor.underflow(underflow);
    
    cout << "Starting simulation of IEEE 754 Floating Point Subtractor" << endl;
    ...
    TestCase test_cases[] = {
        ...
        {9.5f, 5.54f, "Normal numbers (9.5 - 1.0)"},
        {100.0f, 0.01f, "Normal numbers with different scales (100 - 0.01)"},
        {1.0f, 1.0f, "Identity subtraction (1.0 - 1.0)"},
        {5.0f, 3.0f, "Positive - Positive (5.0 - 3.0)"},
        {-4.0f, -5.0f, "Negative - Negative (-4.0 - (-5.0))"},
        {7.5f, 7.5f, "Complete cancellation (7.5 - 7.5)"},
        
        // Zero cases
        {0.0f, 5.0f, "Zero subtraction (0.0 - 5.0)"},
        {0.0f, 0.0f, "Zero by zero (0.0 - 0.0)"},
        {-0.0f, 0.0f, "Negative zero - zero (-0.0 - 0.0)"},
        {-0.0f, -0.0f, "Negative zero - negative zero (-0.0 - (-0.0))"},
        
        // Special cases
        {INFINITY, 5.0f, "Infinity subtraction (inf - 5.0)"},
        {INFINITY, INFINITY, "Infinity - infinity (inf - inf -> NaN)"},
        {-INFINITY, -INFINITY, "Negative infinity - negative infinity (-inf - (-inf) -> NaN)"},
        {INFINITY, -INFINITY, "Infinity - negative infinity (inf - (-inf))"},
        {NAN, 5.0f, "NaN propagation (NaN - 5.0)"},
        {5.0f, NAN, "NaN propagation (5.0 - NaN)"},
        
        // Small number cases
        {1e-20f, 1e-21f, "Very small numbers subtraction"},
        {1e-30f, 1e-31f, "Extremely small numbers"},
        
        // Large number cases
        {1e20f, 1e19f, "Large numbers subtraction"},
        {1e30f, 1e29f, "Very large numbers"},
        
        // Edge cases for normalization
        {1.0f, 1.0f - __FLT_EPSILON__, "Close numbers (1.0 - (1.0 - epsilon))"},
        {1.5f, 0.5f, "Simple subtraction (1.5 - 0.5)"},
        
        // Denormalized number tests (very small numbers)
        {1e-38f, 1e-39f, "Near denormal range"},
        {__FLT_MIN__, __FLT_MIN__, "Minimum float values subtraction"},
        
        // Precision edge cases  
        {1.0f, 0x1p-24f, "1.0 - smallest representable fraction"},
        {0x1.fffffep+127f, 0x1.fffffep+126f, "Near maximum float subtraction"}
    };
    
    // Reset the subtractor
    cout << "Resetting the subtractor..." << endl;
    reset.write(true);
    A.write(0);
    B.write(0);
    sc_start(30, SC_NS);
    ...
    for (const auto& test : test_cases) {
        ...
        bool nan_case = std::isnan(test.a) || std::isnan(test.b) || 
                       (std::isinf(test.a) && std::isinf(test.b) && 
                        ((test.a > 0) == (test.b > 0))); // inf - inf or (-inf) - (-inf)
    ...
    cout << "\nTesting subtraction of very close numbers..." << endl;
    float close_num1 = 1.000001f;
    float close_num2 = 1.000000f;
    
    A.write(floatToHex(close_num1));
    B.write(floatToHex(close_num2));
    sc_start(30, SC_NS);
    
    uint32_t close_result = result.read();
    float close_float;
    memcpy(&close_float, &close_result, sizeof(float));
    
    cout << "  " << close_num1 << " - " << close_num2 << " = " << close_float << endl;
    cout << "  Expected: 0.000001, Actual: " << close_float << endl;
    
    if (fabs(close_float - 0.000001f) < 1e-7f) {
        cout << "  Close numbers test: PASS" << endl;
        passed_tests++;
    } else {
        cout << "  Close numbers test: FAIL" << endl;
    }
    total_tests++;
