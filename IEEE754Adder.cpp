...
// FloatingPointAdder Core - Stage 2
...
SC_MODULE(ieee754add) {
        ...
        add("add"), normalize("normalize") 
    {
        ...
        // Stage 2: Addition (uses registered stage 1 outputs)
        add.A_Mantissa(A_Mantissa_reg);
        add.B_Mantissa(B_Mantissa_reg);
        add.A_Exponent(A_Exponent_reg);
        add.B_Exponent(B_Exponent_reg);
        add.A_sign(A_sign_reg);
        add.B_sign(B_sign_reg);
        add.A_is_nan(A_is_nan_reg);
        add.A_is_zero(A_is_zero_reg);
        add.A_is_inf(A_is_inf_reg);
        add.A_is_denorm(A_is_denorm_reg);
        add.B_is_nan(B_is_nan_reg);
        add.B_is_zero(B_is_zero_reg);
        add.B_is_inf(B_is_inf_reg);
        add.B_is_denorm(B_is_denorm_reg);
        add.reset(reset);
        add.Result_Mantissa(Result_Mantissa_comb);
        add.Result_Exponent(Result_Exponent_comb);
        add.Result_Sign(Result_Sign_comb);
        add.result_is_nan(result_is_nan_comb);
        add.result_is_inf(result_is_inf_comb);
        add.result_is_zero(result_is_zero_comb);
